...
Contains the source code. It's private is so I can draft blog posts without them immediately becoming public.

** 🌐 [[https://github.com/jakewright/jake-wright-net][=jake-wright-net=]] (public)

Contains the generated HTML files, i.e. the output of =hugo build=.
...
A GitHub Action runs the =hugo= build command and commits the output to the public =jake-wright-net= repository.
      ...
      - name: Push changes
        run: |
          ...
          if [[ -n $(git status -s) ]]; then
            COMMIT_MESSAGE=$(git -C ../src log -1 --pretty=%s)
            git add .
...
No build is necessary this time as the GitHub Action has already built the site.
...
Beyond that, there are no obvious advantages to either provider.
